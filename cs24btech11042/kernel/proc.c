...
int kchildsyscount(int pid)
{
  int count = -1;
  struct proc *p = myproc();

  acquire(&wait_lock);
  for (struct proc *pch = proc; pch < &proc[NPROC]; pch++)
  {
    acquire(&pch->lock);
    // Check if pch is a child of p with matching pid
    if (pch->parent == p && pch->pid == pid)
    {
      count = pch->syscallcount;
      release(&pch->lock);
      break;
    }
    release(&pch->lock);
...
int kgetnumchild(void)
{
  ...
  {
    acquire(&pch->lock);
    /* Count only p's children that are not ZOMBIE  */
    if (pch->parent == p && pch->state != ZOMBIE)
    {
      count++;
    }
    release(&pch->lock);
